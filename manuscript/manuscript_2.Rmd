---
title: 'Kin dependency ratios: a population-level measure of the expected kin support burden'
author:
...
**MA Notes: need to mention Caswell's paper here**
...
**MA NOTES: there's been a bunch of papers recently that use stable equations I think (e.g. that one about loss and COVID) so they are probably worth mentioning here.**
...
where $t^*$ is the year in which the woman of age $a$ achieved the mean age at childbearing $\mu$. Thus $m_x(t^*)$ is the fertility rate at age $x$ in the year $t^*$. If age $a$ is less than $\mu$, then $t^*$ is just set to be the current period. Note that the mean age of childbearing may vary depending on the population and time period being considered. 

Indexing to the mean age of childbearing is motivated by the trade-off between using the true cohort rates (which is data intensive and are usually not completely available) and maintaining the computational convenience of using a smaller set of period rates that are completely available. Choosing the index year to be when a mother reaches $\mu$ means that period fertility rates will be correct for the age at which fertility is concentrated. 
...
![Fig.2 Number of surviving kin by age of ego and type of kin, 2010.](figs/kin_country_facet_2010.png){width=600px}
...
![Fig.8 Age distribution, 5-year age-specific fertility rates, and 5-year mortality risks for the world population, 1950.](figs/world_1950_summary.png){width=600px}

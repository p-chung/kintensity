...
for(i in 1:length(countries)){
  ...
  for(j in 1:length(ages)){
    res <- rbind(res, tibble(country = countries[i], 
                             ...
                             meanage = surviving_daughters_mean_age(df = d_period, year = year_test, age_a = ages[j]),
...
```{r}
tfr_period %>% filter(year==2010) %>% 
  left_join(tfr_cohort %>% filter(cohort<1965)) %>% 
  left_join(lx_period %>% filter(year==2010) %>% 
  left_join(lx_cohort %>% filter(cohort<1965))) %>% 
  filter(region %in% countries) %>% 
  mutate(mult = (tfr_cohort/tfr_period)*(lx_cohort/lx_period))  %>% 
  ggplot(aes(2010 - cohort, mult, color = region)) + geom_line()
```
...
tfr_period %>% 
  ...
  group_by(region) %>% 
  arrange(region) %>% 
  mutate(tfr_std = tfr_period/tfr_period[year==1950], 
         ratio = (tfr_period[year==1975])/tfr_period[year==2010]) %>% 
